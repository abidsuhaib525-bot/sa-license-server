...
(function legacyLicenseBridge() {
  ...
  const PREMIUM_LICENSE_STATE = {
    ql_license_valid: true,
    license_valid: true,
    ql_license_key: "LAST-ZONE-PREMIUM-LICENSE-KEY",
    license_key: "LAST-ZONE-PREMIUM-LICENSE-KEY",
    ql_license_id: "premium-license-id",
    ql_license_status: "unlimited",
    ql_license_plan: "unlimited",
    ql_user_name: "Suhaib Abid",
    ql_user_email: "premium@last.zone",
    ql_expires_at: "9999-12-31T23:59:59.000Z",
    ql_activated_at: "2026-01-01T00:00:00.000Z",
    ql_device_id: "lz_device_premium",
    ql_device_id_fallback: "lz_device_premium",
    device_id: "lz_device_premium",
    ql_session_id: "lz_session_premium",
    ql_synced_sessions: [],
    ql_native_chat: false,
    ql_bypass_active: true,
    __ql_bypass_active: "1"
  };
  try {
    chrome.storage.local.set(PREMIUM_LICENSE_STATE, function () {});
  } catch (e) {}
  try {
  ...
  async function validateLegacyLicense(rawKey, options) {
    options = options || {};
    const key = String(rawKey || "LAST-ZONE-PREMIUM-LICENSE-KEY").trim();
    const deviceId = options.deviceId ? String(options.deviceId) : "lz_device_premium";
    const sessionId = options.sessionId || "lz_session_premium";
    const localState = {
      ql_license_valid: true,
      ql_license_key: key,
      ql_license_id: "premium-license-id",
    ...
    try {
      ...
      if (typeof startLocalExpiryCheck === "function") {
        startLocalExpiryCheck();
      }
    } catch (error) {}
    return localState;
  }
  function showLicenseGateNow(reason) {
    // Stubbed to prevent license gate display
  }
  async function clearLicenseAndShowGate(reason) {
    // Stubbed to prevent clearing premium license
  }
  async function revalidateStoredLicense() {
    // Stubbed to prevent server revalidation
  }
  ...
  function handleLogoutClick(event) {
    // Stubbed to prevent logout
  }
  function startRevocationMonitor() {
    // Stubbed to prevent revocation checks
  }
